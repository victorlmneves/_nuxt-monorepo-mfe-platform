...
// export default defineEventHandler((event) => nuxtAuthHandler(event))
  ...
  callbacks: {
    ...
    async jwt({ token, account, trigger, session }) {
      ...
      let expiryDate: number =
        (account?.expires_at as number) ?? (token?.expiresAt as number) ?? 0;
      expiryDate *= 1000;
      ...
      if (
        trigger === "update" &&
        (session as unknown as { forceRefresh?: boolean })?.forceRefresh
      ) {
    ...
    async session({
      ...
      (session as ICustomSession).expiresAt = token.expiresAt as
        | number
        | undefined;
...
//   console.log('API handler hit!', event)
//   return 'OK'
// })
